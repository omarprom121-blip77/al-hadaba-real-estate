...
async function getItems(kind) {
  const r = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || ''}/api/public`, { cache: 'no-store' });
...
export default async function ContentPage({title,description,kind}) {
  ...
  <section className="section"><div className="container"><div className="cards">{items.map(x=><article className="card" key={x._id}>{x.image && <img className="card-media" src={x.image} alt={x.title}/>} {x.video && <video className="card-media" src={x.video} controls preload="metadata" playsInline aria-label={`فيديو ${x.title}`}/>} {!x.image&&!x.video&&<div className="card-img"/>}<div className="card-body"><h3>{x.title}</h3><p>{x.description}</p></div></article>)}{loadError ? <div className="empty">{loadError}</div> : !items.length && <div className="empty">لا يوجد محتوى منشور حاليًا. يمكنك إضافته من لوحة التحكم.</div>}</div></div></section>
